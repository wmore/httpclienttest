...
import com.wangyue.http.SanXiaUrlUtils;
...
@Component
public class DataManagerByHTTP {
    ...
    @Resource
    private SanXiaUrlUtils sanXiaUrlUtils;

    private String[] gradeList = new String[]{"2014", "2015", "2016", "2017", "2018", "2019"};
    ...
    private void syncDepartmentTable() {
        Element xmlRoot = departmentParser.initDepartmentFromString(sanXiaUrlUtils.getDepartment(),"DepartmentTable");
        List<Department> list = departmentParser.listNodes(xmlRoot);
    ...
    private void syncSpecialtyTable() {
        Element xmlRoot = departmentParser.initDepartmentFromString(sanXiaUrlUtils.getSpecialInfo(), "SpecialTable");
        List<SpecialInfo> list = specialInfoParser.listNodes(xmlRoot);
    ...
    private void syncClassTable() {
        for (String grade:gradeList) {
            Element xmlRoot = departmentParser.initDepartmentFromString(sanXiaUrlUtils.getClassInfo(grade), "ClassTable");
            List<ClassInfo> list = classInfoParser.listNodes(xmlRoot);
    ...
    private void syncTeacherTable() {
        Element xmlRoot = departmentParser.initDepartmentFromString(sanXiaUrlUtils.getTeacher(), "TeacherTable");
        List<Teacher> list = teacherParser.listNodes(xmlRoot);
...
//            tTeacher.setTelephoneNumber();
    ...
    private void syncStudentTable() {
        for (String grade:gradeList) {
            Element xmlRoot = departmentParser.initDepartmentFromString(sanXiaUrlUtils.getStudent(grade),"StudentTable");
            List<Student> list = studentParser.listNodes(xmlRoot);
